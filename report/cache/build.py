...
class BuildCache:
    ...
    def needs_update(
        self, data_type: str, run_id: int | None, new_data: Any
    ) -> bool:
        """
        检查数据是否需要更新
        ...
        try:
            ...
            if needs_update:
                logger.debug(
                    "数据变更检测到，需要更新: %s", fingerprint_path.name
                )
            else:
                logger.debug(
                    "数据未变更，跳过: %s", fingerprint_path.name
                )
            return needs_update
    ...
    def update_fingerprint(
        self, data_type: str, run_id: int | None, data: Any
    ) -> None:
        """
        更新数据指纹
    ...
    def needs_frontend_rebuild(self, web_dir: Path) -> bool:
        ...
        Returns:
            bool: True 表示需要重新构建
        """
        src_hash = self.compute_dir_hash(web_dir / "src") + self.compute_dir_hash(
            web_dir / "public"
        )
