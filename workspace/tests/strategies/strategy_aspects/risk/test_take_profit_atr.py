"""建议型切面测试 — 验证 with_take_profit_atr

覆盖:
  - ATR 指标自动注册到 data_requirements
  - 无持仓透传
  - 多头/空头 ATR 止盈触发（写入 aspects.risk）
  - 多头/空头 ATR 止盈不触发
  - ATR 值为空/零时跳过
  - diagnostics 字段完整性
  - 多个 AOP 装饰器叠加
...
class TestExitTakeProfitATR:
    ...
    def test_long_atr_take_profit_triggered(self):
        ...
        state = _make_state(
            direction=TRADE_DIRECTION_LONG, entry_price=100.0, atr_take_profit_multiplier=3.0
        )
        ...
        assert ctx.aspects.risk.take_profit.exit[0].detail["type"] == "atr"
    ...
    def test_short_atr_take_profit_triggered(self):
        ...
        state = _make_state(
            direction=TRADE_DIRECTION_SHORT, entry_price=100.0, atr_take_profit_multiplier=3.0
        )
    ...
    def test_atr_diagnostics(self):
        ...
        assert ctx.aspects.diagnostics.get("current_close") == 107.0
...
class TestCombinedDecorators:
    ...
    def test_fixed_take_profit_triggers(self):
        """固定比例止盈触发 → aspects.risk 包含固定比例止盈"""
        state = State(
            symbol="TEST",
            period="1m",
        ...
        # ATR 不触发，但固定比例止盈触发
        ctx = _MockMultiCtx(close=103.0, atr_value=2.0)  # 涨 3%
        signal = self.strat.on_bar(state, ctx)
        assert signal is _ON_BAR_RETURN
    ...
    def test_atr_take_profit_triggers(self):
        """ATR 止盈触发 → aspects.risk 包含 ATR 止盈"""
        state = State(
            symbol="TEST",
            period="1m",
            strategy_config=_ATRParams(atr_take_profit_multiplier=3.0),
            capital=100000.0,
            contract_size=10,
            position=StrategyPosition(
                direction=TRADE_DIRECTION_LONG,
        ...
        # 固定比例不触发，但 ATR 止盈
        ctx = _MockMultiCtx(close=107.0, atr_value=2.0)  # 100+2*3=106
        signal = self.strat.on_bar(state, ctx)
        assert signal is _ON_BAR_RETURN
        risk_names = [r.name for r in ctx.aspects.risk.all_reasons]
        assert SIGNAL_TAKE_PROFIT in risk_names

    def test_both_take_profit_trigger(self):
        """固定比例和 ATR 止盈同时触发 → aspects.risk 同时包含两者"""
        state = State(
            symbol="TEST",
            period="1m",
            strategy_config=_ATRParams(take_profit_ratio=0.01, atr_take_profit_multiplier=3.0),
            capital=100000.0,
            contract_size=10,
            position=StrategyPosition(
                direction=TRADE_DIRECTION_LONG,
        ...
        ctx = _MockMultiCtx(close=107.0, atr_value=2.0)
        # 固定比例先触发(1%), ATR 止盈也需要 106
        signal = self.strat.on_bar(state, ctx)
        assert signal is _ON_BAR_RETURN
        risk_names = [r.name for r in ctx.aspects.risk.all_reasons]
        assert SIGNAL_TAKE_PROFIT in risk_names
        assert len(ctx.aspects.risk.take_profit.exit) == 2
