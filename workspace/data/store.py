...
重构说明（阶段 8）：
  - 报表视图查询（`get_run_summary` / `get_backtests_for_run` / `get_equity_data`）
    及其辅助（`_filter_by_best_trial` / `_update_best_for_symbol`）迁出到 `data/report_queries.py`，
    DataStore 回归数据库基础 CRUD。
"""
