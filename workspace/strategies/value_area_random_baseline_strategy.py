...
# 文件级元信息：
# - 创建背景：R29 随机入场复验需要长期可复用的 VA 结构随机基准，而不是继续依赖归档中的临时实验策略。
# - 用途：在 VA reacceptance baseline 的事件、止损和退出口径上随机化入场，用于判断结构入口是否优于随机入场。
# - 注意事项：该策略用于同 runner 下的相对基准比较；当前复用 baseline 私有方法，若长期扩展应抽取共享 VA 事件组件。
import random
...
from .value_area_reacceptance_baseline_strategy import (
    TradeInfo,
    ValueAreaLevels,
...
@dataclass
class ValueAreaRandomBaselineParams(ValueAreaReacceptanceBaselineParams):
    random_seed: int = 42
    random_baseline_mode: RandomBaselineMode = "direction_matched"
    random_direction_mode: RandomDirectionMode = "same"
    random_entry_probability: float = 1.0
    random_breakout_extra_ticks: int = 10
...
class ValueAreaRandomBaselineStrategyCore(Strategy[ValueAreaRandomBaselineParams]):
    ...
    def __init__(self) -> None:
        self._base = ValueAreaReacceptanceBaselineStrategyCore()
    ...
    def _random_breakout_extreme(
        ...
        max_extra_ticks = max(config.random_breakout_extra_ticks, 0)
        extra_ticks = rng.randint(0, max_extra_ticks) * config.price_tick
        if side == "long":
            return min(bar.low, bar.close - buffer - extra_ticks)
        return max(bar.high, bar.close + buffer + extra_ticks)

    @staticmethod
    def _base_state(state: State[ValueAreaRandomBaselineParams]) -> State[ValueAreaReacceptanceBaselineParams]:
        return cast(State[ValueAreaReacceptanceBaselineParams], state)
