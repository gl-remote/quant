"""with_cooldown_after_stop_loss — 止损后冷静期建议切面"""
...
def with_cooldown_after_stop_loss(minutes: int) -> Callable[[T], T]:
    """类装饰器工厂：止损成交后在指定分钟内产生入场阻断建议。"""
    ...
    return _entry_block_aspect("stop_loss", SIGNAL_TRADE_COOLDOWN, _trigger)
