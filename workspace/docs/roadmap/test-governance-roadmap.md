...
- `scripts/test.sh` 当前已经承担过多职责，后续应先机械拆分，再增加 coverage / integration / local_data 能力。
...
- 真实行情数据、`.quant_shared_data`、本机 SQLite 状态不得成为 fast suite 的隐式依赖。
...
| `scripts/test/python.sh` | `run_lint`、`run_format`、`run_type`、`run_unit`、后续 `run_coverage` |
...
如不想增加太多 stage，也可以先在 `unit` 内固定排除：

```bash
-m "not slow and not local_data"
```

但要谨慎：这会改变当前 `run_unit` 行为，必须单独提交，并在提交说明中明确。
...
- `slow`：耗时明显的测试。
- `local_data`：依赖 `.quant_shared_data` 或本机真实行情。
...
- helper 不应依赖真实数据、不应隐式读 `.quant_shared_data`。
...
核心判断：DataFeed 在完成数据构建后，`build_context` / `get_data` / 高周期聚合视图应尽量表现为“给定输入数据 + 给定 current_time → 确定输出”的纯函数。也就是说，测试重点不应只证明 DataFeed 能跑通，而应证明同一份构建好的 feed 在任意读取顺序、重复读取、不同窗口组合下都返回一致且不偷看的结果。
...
建议 CI 不依赖本机 `.quant_shared_data`。真实数据测试应显式标记并在专门环境运行。
...
- `scripts/test.sh` 尚未拆分
- coverage stage 尚未建立
- `workspace/tests/helpers/` 尚未建立
- 核心不变量测试仍待系统补充
...
- `scripts/test.sh` 已拆分，但外部命令兼容。
- pre-commit 不需要修改 entry，仍按业务域增量触发。
- coverage 有专用入口，不污染默认 pytest。
- `slow` / `local_data` 与 fast suite 清晰隔离。
