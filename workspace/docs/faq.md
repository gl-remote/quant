...
A: 默认存储在 `project_data/market_data/csv/` 目录，文件格式为 `{symbol}.{provider}.{interval}.csv`。
...
A: 存储在 SQLite 数据库 `project_data/database/quant_shared.db` 中。
...
```bash
uv run python main.py backtest --symbol DCE.m2509 --strategy ma --start 2024-01-01 --end 2024-12-31
```
...
```bash
uv run python main.py test --strategy ma
# 或
