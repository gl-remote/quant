...
1. 在 Q2 中新增"时间对齐规则"小节，详细说明：
   - 假设所有 K线的时间戳表示**周期开始时间**（如 09:30 表示 09:30-09:31 的 1m K线）
   - 周期转换采用**时间窗口聚合**，而非简单按根数聚合
   - 1m → 5m 的时间窗口为 `[T, T+5min)`，其中 T 是 5分钟对齐的起始时间（09:30, 09:35, 09:40 等）
   - 聚合后的高级周期 K线时间戳取窗口起始时间
   - 当低级周期数据覆盖了完整的高级周期窗口时，才触发聚合
   - 非标准交易时间（如夜盘）按实际交易时间处理，不强行填充缺失数据
