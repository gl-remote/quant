...
**编制日期**: 2026-05-31  
**文档状态**: 送审稿  
**修订说明**: 根据技术审计建议进行优化，改进并发安全、指标计算状态管理、向后兼容性
...
#### 问题 1：并发安全设计缺陷（高风险）✅ 已修复

**问题描述**：
- `DataFeedCache.get_or_create` 缺少锁保护，多线程场景下可能出现同时创建同一 symbol 的多个 DataFeed 实例

**修复方案**：
- 在 `DataFeedCache` 中添加 `_lock: threading.RLock` 字段
- 在 `get_or_create` 方法中使用 `with self._lock` 保护关键区域
- 添加 `__init__` 方法初始化锁和 `_datafeeds` 字典

---

#### 问题 2：指标计算触发时机不明确（中风险）✅ 已修复

**问题描述**：
- 缺少指标计算状态跟踪机制，无法判断哪些指标已计算、最后计算到哪个位置
...
#### 问题 3：周期转换触发逻辑不完整（中风险）✅ 已修复

**问题描述**：
- 缺少周期转换关系的存储结构，无法清晰区分哪些是派生周期、哪些是源周期

**修复方案**：
- 在 `DataFeed` 中添加周期转换配置：
  - `_period_conversions: Dict[Tuple[str, str], Callable]`：(源周期, 目标周期) → 转换函数
  - `_derived_periods: Dict[str, str]`：目标周期 → 源周期（用于标记哪些是派生的，避免与直接数据源冲突）

---

#### 问题 4：向后兼容性问题（中风险）✅ 已修复

**问题描述**：
- 原方案直接修改接口，不考虑向后兼容，旧策略需要强制迁移
...
| 性能考虑 | ⚠️ 一般 | Pandas append 性能在大数据量下可能有问题 |
