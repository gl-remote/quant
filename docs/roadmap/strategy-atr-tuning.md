...
**有搞头，但方向要收敛；当前主要问题不是自然时间窗口一定不够，而是信号密度远低于目标。**

如果目标策略是“一周发现 3~5 次机会，平均持仓 1~2 天”，那么一个月数据理论上应能产生约 12~20 笔交易，已经足以做第一轮方向性判断。当前每个合约只有 1 笔交易，说明现有 ATR 入场结构没有达到目标交易频率；后续优先修正信号密度，而不是简单归因于数据月份不足。
...
#### P1.5 信号密度重构首轮验证（2026-06-26）

> 框架层问题已单独记录：[framework-indicator-history-issues.md](./framework-indicator-history-issues.md)

本轮从 `dev/0.5` 新开 `feature/atr-signal-density`，目标是先验证“一周 3~5 次机会、平均持仓 1~2 天”的交易节奏是否能被当前策略结构支持。

实现调整：

1. 入场从“15m 趋势 + 5m MACD + 5m KDJ 同时满足”改成“15m 趋势背景 + 5m KDJ 中线触发”；
2. KDJ 触发通过 `state.extra` 保存最近 5m KDJ，避免运行时 `PeriodDataView` 只能可靠读取最新指标导致回看值为 `None`；
3. 新增 `time_stop_bars = 48`，约束最长持仓约 1 个交易日，避免单笔持仓占用整个样本窗口；
4. 5m KDJ 需求 lookback 从 `KDJ.window + 1` 提高到 `20`，避免 KDJ 预热不足导致最新值持续为 `NaN`。

关键验证过程：

- run `r4`：KDJ 状态记忆实现后仍为 `0` 笔，定位为 5m KDJ 历史读取/预热问题；
- run `r7`：加入 `time_stop_bars = 48` 后为 `3 / 3 / 1` 笔，说明退出释放开始生效，但密度仍不足；
- run `r13`：修正 KDJ lookback 后为 `3 / 1 / 5` 笔，较 P1 的 `1 / 1 / 1` 有改善，但距离一个月约 `12~20` 笔仍不足。
...
结论：本轮证明“数据自然时间不是唯一问题”，当前工具链和策略结构确实能通过放松触发提高交易数；但 `3 / 1 / 5` 仍未达到目标。下一轮应继续围绕交易节奏重构，而不是做 ATR 参数搜索。

下一轮优先级：

1. 继续以“一周 3~5 次机会、平均持仓 1~2 天”为目标，提高真实成交密度到一个月约 12~20 笔；
2. 优先检查出场机制和反向/再入场释放，而不是继续放宽到无约束高频信号；
3. 增加持仓周期统计，确认时间止损是否真的把平均持仓压到 1~2 天；
4. 交易密度达标后，再进入 ATR 参数搜索和 Walk-Forward；
5. 暂不根据当前 `3 / 1 / 5` 小样本结果调参或引入 DMI/ADX。
...
目标：先让策略达到“一周 3~5 次机会、平均持仓 1~2 天”的交易节奏，再评价胜率和盈亏比。

建议方向：

1. 保留 15m 趋势背景，但允许趋势背景更平滑，避免频繁反向；
2. 5m 触发从“MACD 与 KDJ 同时满足”改为更容易触发的回调完成/再启动结构；
3. 将 KDJ 从硬过滤改为回调状态记忆：先出现回调，再等待 MACD 或价格重新转强；
4. 增加持仓周期统计：平均持仓、最长持仓、1 日内退出比例、超过 2 日退出比例；
5. 用交易密度作为第一层验收，不先追求收益最优。

验收标准：

- 一个月样本约 12~20 笔交易，或至少接近每周 3~5 次机会；
- 平均持仓周期接近 1~2 天；
- 单笔平均盈亏仍显著大于交易成本；
- 交易密度达标后再进入 ATR 参数搜索。
...
1. 先按目标信号密度重构 ATR 入场，目标是一周 3~5 次机会、平均持仓 1~2 天；
2. 跑一个月样本，先看交易次数、平均持仓、成本敏感性；
3. 如果交易密度仍不足，继续放松入场触发，而不是优化 ATR 参数；
4. 交易密度达标后，再调 ATR 止损、止盈、移动止盈；
5. 然后加时间止损和 ATR 仓位；
6. 最后再加 ADX/DMI 辅助过滤。
