...
- `3659e3f` — Commit C：拆 `tqsdk.py` 为 `test.py` + `live.py`，抽取 `cli/workflows/tqsdk_realtime.py`（`TqsdkRealtimeWorkflow` + `TqsdkRealtimeRequest`）。每个命令文件拥有自己的 argparse 定义，所有 Namespace → Request 翻译发生在 commands 层。
...
cli/commands/{backtest,export,test,live,report}.py
  ...
  ↓ 纯净 Request 对象
cli/workflows/{backtests_run,tqsdk_realtime,backtests_lifecycle}.py
                                     ← 业务编排，不感知 argparse
```
