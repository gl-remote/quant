...
> ⚠️ **阅读顺序建议**：本文档 §一–§六 讲的是"**单因子独立盈利**"的硬判据——
> 这是极高要求（$se_{\text{target}} \approx 0.05$ 对应把 20h 窗口回归的
> se 0.224 压 4.8 倍）。**接近达标但差一点的因子不应被错杀**，
> 请务必配合 [§七 分级因子管理](#七分级因子管理--单因子盈利不是唯一去处)
> 阅读——§七 把二值 accept/reject 升级为四档评级 L1/L2/L3/L4，
> 大多数"L2 边缘通过"因子可以进入融合池，共同兑现主题目标。
...
> 💡 **分级管理提示**（详见 §七）：**这里的 reject 不是二值判决**。
> $\widehat{se}$ 落在 $(se_{\text{target}}, 1.2 \cdot se_{\text{target}}]$ 归 **L2 · 边缘通过**（进融合池）；
> $(1.2, 3.0] \cdot se_{\text{target}}$ 归 **L3 · 辅助过滤**（只做组合信号）；
...
> 💡 **分级管理提示**（详见 §七）：覆盖率不足**可以通过融合稀释解决**——
> 一个 L2 因子（fire 少但精度高）与另一个 L2/L3 因子做 OR 组合就能补齐 Gate 2。
> 单因子 Gate 2 失败**默认降级为 L2**（有条件融合），只有当 fire 稀疏到融合也补不齐（$N_{\text{year}}/N_{\text{year}}^\ast < 0.10$）才归 L4。
...
> 💡 **分级管理提示**（详见 §七）：$r$ 落在 $[0.20, 0.40)$ 归 **L3 · 辅助过滤**
> （方向对但精度不足，可作过滤器）· 只有 $r < 0.20$ 才真正是"方向错"陷阱、归 L4。
> $r \ge 0.40$ 但 CI 边界不显著归 **L2**。
...
> 💡 **分级管理提示**（详见 §七）：本表的 `accepted=False` 是**"未达 L1"** 的意思，
> 不等于 L4 归档。下游读到 `run_screening()` 返回值必须**结合 §7.1 四档表**决定
> 归入 L2（边缘通过 · 融合池）/ L3（辅助过滤池）/ L4（反例登记）中的哪个。
> 具体分级映射见 §7.4。
...
## 七、分级因子管理 · 单因子盈利不是唯一去处

前六节的三层 gate + 边界条件是"**单因子独立盈利**"的硬判据——通过意味着"这一个因子
单独配合塑形容器就能实盘正 $E_{\text{net}}$"。这是极高要求：

- shaping-theory §2.23.6.5 的 $se_{\text{target}} \approx 0.05$ 对应"能把 20h 窗口回归的 se
  0.224 压 4.8 倍"——单信号很难做到
- 但**很多单独达不到 $se_{\text{target}}$ 的信号仍然有价值**——只是需要**融合**才能兑现

本节把"是否单独盈利"从"accept / reject"的**二值判断**升级为**四档评级**，
让接近达标的因子不被错杀，同时给出后续融合、事件筛选、辅助过滤的组织方式。

### 7.1 四档因子评级 · L1 / L2 / L3 / L4

| 等级 | 通过条件 | 语义 | 下游用途 |
|---|---|---|---|
| **L1 · 独立盈利** | 全部 gate 通过 · 严格版（bootstrap CI 也过） | 单因子实盘正 $E_{\text{net}}$ | ✅ 直接进入下游 OOS / 实盘子主题 |
| **L2 · 边缘通过** | 点估计过 · CI 边界 · 或点估计在 $se_{\text{target}}$ 的 1.0-1.2 倍 | 单独实盘不稳，但**融合池的核心贡献者** | ✅ 进入融合候选池 · 优先与其他 L2/L3 组合 |
| **L3 · 部分通过** | Gate 1 挂但差距 < 3×（$\widehat{se} \le 3 \cdot se_{\text{target}}$）· Gate 2/3 通过 | 精度不足以独立兑现，但**方向对**——秩相关合格 | ⚠️ 进入辅助过滤池 · 只能配合 L1/L2 使用（比如"若 L1 fire 且 L3 同意才入场"） |
| **L4 · 归档反例** | Gate 1 严重失败（差距 ≥ 3×）· 或 Gate 0 泄漏 · 或方向审计失败 | 结构性不合 · 不进任何池 | ❌ 归 `rejected_factors.md` · 反例登记 |

**"差一点"的定义**：
- $se_{\text{target}} \le \widehat{se} \le 1.2 \cdot se_{\text{target}}$ → **L2 · 边缘**
- $1.2 \cdot se_{\text{target}} < \widehat{se} \le 3 \cdot se_{\text{target}}$ → **L3 · 辅助**
- $\widehat{se} > 3 \cdot se_{\text{target}}$ → **L4 · 归档**

### 7.2 L1/L2/L3 因子的组合规则

**L1**：单独就能上下游 OOS · 融合池里作为"锚点因子"

**L2 + L2**：两个 L2 组合成 fusion factor 后重跑 Step 0-7；
若融合后 $\widehat{se}_{\text{fusion}} \le se_{\text{target}}$ → 融合体升级为 L1

**L2 + L3** / **L3 + L3**：不允许融合成 L1 · 但可以做**AND 过滤器**（"L1 fire ∧ L3 fire 才入场"），
把年入场次数压低换取单笔期望上升 · 用途属"信号稀释保险"

**任意 L4** · 不允许出现在任何组合里

### 7.3 融合因子的评估流程

融合 $\hat{x}_{\text{fusion}} = w_1 \cdot \hat{x}_1 + w_2 \cdot \hat{x}_2 + \ldots$（权重可 grid search / 岭回归）
必须**完整重跑 Step 0-10**，视为新的独立候选（新 slug、新登记档）。**禁止**：

- 用融合后过 Gate 1 → 直接把每个成分因子标为 L1
- 只跑单点 se · 不跑 bootstrap CI
- 融合权重从测试集里学习（属 Step 8 反模式"训练集回代 Gate 3"）

### 7.4 修订 §四 Step 4/5/6/7 的"reject"措辞

前四节的三层 gate 判决默认写的是 "reject"。**在分级管理下应改读为**：
...
**Step 7 终审函数 `run_screening()` 的返回值语义顺势升级**——
`accepted / reject_reason` 仍然给出，但**下游必须结合 §7.1 的 L1/L2/L3/L4 表**再判断因子归入哪个池，
而不是简单按 accepted 布尔值取舍。

### 7.5 融合前提 · 因子独立性检验

融合前必须先检查 L2/L3 因子之间是否**独立**——否则融合无收益（信息重复）。

**判据**（简版）：因子输出之间 Spearman 相关 $r_{\text{cross}}$：

- $|r_{\text{cross}}| < 0.30$ → **独立** · 融合有效
- $0.30 \le |r_{\text{cross}}| < 0.70$ → **弱相关** · 融合可能有效 · 需 bootstrap 验证融合后 se 提升
- $|r_{\text{cross}}| \ge 0.70$ → **强相关** · 融合几乎无收益 · 不建议

**方法**：`research.screening._spearman_rank` 或 `scipy.stats.spearmanr`（本主题不硬性依赖 scipy）。

### 7.6 定期 review · 反例池升级机制

`rejected_factors.md` 里的 L3/L4 因子不是"永久死亡"——每 6 个月 review 一次：

- 数据积累后 se 可能下降（样本量效应）· L3 → L2 可能
- 新的融合组合可能包含旧的 L3 → 重新进入活跃池
- L4 归档为反例登记的具体原因（Gate 0 泄漏 / bias / 精度不足），未来 AI/研究员可查

Review 结论写入 `research-status.md` KF 清单，作为主题的活跃维护证据。
...
| 本主题 | 传统因子库 | 备注 |
|---|---|---|
| L1 · 独立盈利 | Alpha factor · in production | 直接部署 |
| L2 · 边缘通过 | Alpha candidate · in pipeline | 融合池 |
| L3 · 辅助过滤 | Filter / risk factor | 组合信号 |
| L4 · 归档反例 | Rejected / lookahead / stale | 反例登记 |

**核心差异**：本主题的 L 等级是**由 $se_{\text{target}}$ 反解的连续阈值**决定，不是拍脑袋分档。
$se_{\text{target}}$ 一旦按 KF-27 反解、就自动锁定四档边界。

### 7.8 分级判决示例

假设玉米 1h（$se_{\text{target}} = 0.047$）上跑 4 个候选，得到：

| 候选 | $\widehat{se}$ | Gate 2 | Gate 3 $r$ | 方向审计 | **等级** | 处理 |
|---|---|---|---|---|---|---|
| Hurst-1h | 0.045 | ✅ | 0.55 | neutral | **L1** | ✅ 进 OOS |
| RV-breakout | 0.055 | ✅ | 0.45 | neutral | **L2** | 融合池 · 与 Hurst 融合验证 |
| ATR-turn | 0.10 | ✅ | 0.35 | neutral | **L3** | 辅助过滤 · 只与 L1/L2 组合 |
| 20h-regression | 0.224 | ❌ | 0.10 | neutral | **L4** | 归档反例登记 |

**观察**：即使 Hurst 单独 L1，Hurst + RV 融合后 $\widehat{se}_{\text{fusion}}$ 可能进一步降到 0.035 → 
融合体升级为 L1' · Sharpe 可能显著高于单 Hurst · 这是分级管理的直接收益。

***

**注**：前四节 Step 0-10 的原文措辞（"reject" / "失败 → reject"）保留不动，
读者请**在心里对照 §7.4 表格**做等级映射。若未来实验反复出现 L2/L3 场景，
本主题会考虑把 §四 Step 7 的返回值 API 直接扩展成 `Level` 枚举。
