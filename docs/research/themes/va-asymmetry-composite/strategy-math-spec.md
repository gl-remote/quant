...
> 本规格是 `va-asymmetry-composite` 策略的**数学契约**：单独阅读即可复现当前策略行为，不依赖代码或聊天上下文。所有参数集中在 §0，正文公式引用 §0 符号。实验过程、候选方案比较与结论见归档 `../../workbench/va-asymmetry-composite-p0-p9-summary.md`，不在本契约内重复。
