...
| [factor-filtering-and-dgp-boundary.md](factor-filtering-and-dgp-boundary.md) | 因子筛选后条件过程的漂移 $b_A(x)$ 满足什么条件时，GBM 塑形闭式仍然成立？当漂移状态依赖（OU）/ 时变切换 / 非半鞅（fBm）时，barrier 结论如何替换？识别门要求是什么？ | 入库 2026-08-02 |

---

## 阅读顺序

四份文档**互补**、可独立阅读，构成主题的四条稳定支柱（第五条 `factor-filtering-and-dgp-boundary` 为适用边界补丁，横切前四者）：
...
- **factor-filtering-and-dgp-boundary.md** —— 适用边界契约，回答"**因子筛选后 GBM 塑形闭式何时仍成立**"，9 章 + 2 附录，用一维扩散尺度函数统一 GBM 常数漂移通道与 OU 状态依赖通道，给出 GBM 闭式 iff $b_A$ 常数的边界定理与套闭式前的识别门。

**问题域层级**：前四者构成"何时有 alpha → alpha 长啥样 → alpha 怎么兑现 → alpha 随时代如何衰减"的完整逻辑链——前三者做**截面判据**（当下市场强度 $s$ 给定），第四者做**时间演化判据**（$s$ 如何随 Hurst 演化）。第五者是**元层判据**：任何因子入场前，先识别条件漂移 $b_A$ 的类别（零 / 常数 / 状态依赖 / 切换 / fBm），再决定套用哪套 barrier 闭式；它同时为 `mean-reversion-barrier-duality` 主题的 OU 通道提供了与本主题 GBM 通道的统一母公式。
