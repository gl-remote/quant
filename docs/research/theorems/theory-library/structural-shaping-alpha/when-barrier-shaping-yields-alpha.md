...
### 10.5 方向概率推广（通道 A/B 混合）

**动机**：§10.1–§10.4 的 KF-27 设定在 DirRandom baseline（$p = 1/2$，通道 B）下成立；实际工程中方向识别器能提供弱方向 edge（$p > 1/2$，如 KF-19 EMA20 aligned 组实证 $p \approx 0.55$）。本节把 KF-27 的混合公式推广到任意方向命中率 $p \in [1/2, 1]$，给出闭式表达并展示 $p$ 对最优参数 $(K_S^\ast, K_T^\ast, \tau_\text{q}^\ast)$ 的定量影响。

**定义 10.5（方向命中率 $p$）**：设入场时方向信号预测 drift 为正；真实 drift 在入场段为正的概率为 $p$，为负的概率为 $1-p$。DirRandom baseline 对应 $p = 1/2$；完美方向信号对应 $p = 1$。

**命题 10.6（$p$-混合期望）**：在定义 10.5 下，单笔首达 gross 期望为

$$
\boxed{\;E_{\text{gross}}^{(p)}(x; K_S, K_T) = p \cdot g(+2x) + (1-p) \cdot g(-2x)\;}
$$

其中 $g(\lambda) = K_T P_{\text{win}}(\lambda) - K_S (1-P_{\text{win}}(\lambda))$ 是单边 drift 首达期望（命题 9.2 的单边版本）。当 $p = 1/2$，退化为 KF-26 混合公式

$$E_{\text{gross}}^{\text{mix}} = \tfrac{1}{2}[g(+2x) + g(-2x)];$$

当 $p = 1$，退化为已知方向的纯通道 A 期望 $g(+2x)$。

**证明.** 以 $p$ 概率 drift 与信号同向（$\lambda = +2x$），以 $1-p$ 概率反向（$\lambda = -2x$）；线性期望直接相加即得。$\blacksquare$

**推论 10.7（$p$-混合的二阶展开）**：小 $x$ 下，

$$
E_{\text{gross}}^{(p)}(x) \approx \underbrace{(2p-1) \cdot x \cdot \frac{K_S K_T (K_T + K_S)}{K_T + K_S}}_{\text{通道 A 线性项}} + \underbrace{\frac{x^2}{3} K_S^3 R(R-1)}_{\text{通道 B 二次项（命题 11.2）}}
$$

- $p = 1/2$：线性项为零，还原命题 11.2 的纯二次项；
- $p > 1/2$：出现正线性项——通道 A 的方向 edge 让首达期望在 $x \to 0$ 时仍有正贡献；
- $p = 1$：线性项最大，最优 $K_S$ 可显著收紧（见命题 10.8）。

**命题 10.8（$p$ 对最优参数的定性影响）**：固定 $(D, c, \sigma_\text{bar}, T_\text{year})$，$p$ 从 $1/2$ 单调增至 $1$ 时：

| 参数 | $p \to 1$ 的极限方向 | 机制 |
|------|---------------------|------|
| $K_S^\ast$ | **单调减小**（紧止损）| 方向 edge 让被噪声扫损概率降低，不需容忍反向 drift |
| $K_T^\ast / K_S^\ast = R^\ast$ | **单调减小**（降低非对称性）| 通道 B 的尾部补偿需求减弱 |
| $\tau_\text{q}^\ast$ | **单调增大**（提高强段门槛）| 线性项让弱段也有利可图，不需放低门槛凑笔数 |
| $\mathrm{Sharpe}_\text{年}^\ast$ | **单调增大** | 双通道叠加 |

**实证锚点 · 玉米 1h 方向概率扫描**（$D = \mathrm{FoldedNormal}(0.072, 0.100)$，$c = 0.077$，$T_\text{year} = 1625\,\text{h}$，扩网格 $K_S \in [0.5, 6]$、$R \in [1, 5]$、$\tau \in [0.02, 0.50]$，数值积分闭式解）：

| $p$ | $K_S^\ast$ | $K_T^\ast$ | $R^\ast$ | $\tau_\text{q}^\ast$ | $N_\text{年}$ | $E_{\text{net}}$/笔 | $\mathrm{Sharpe}_\text{年}$ | 年化@r=1% |
|-----|-----------|-----------|----------|----------------------|---------------|---------------------|----------------------------|-----------|
| 0.50 | 3.5 | 17.5 | 5.0 | 0.06 | 15.6 | +2.73 | +1.12 | +12.2% |
| 0.55 | 3.5 | 17.5 | 5.0 | 0.06 | 15.6 | +3.34 | +1.33 | +14.9% |
| 0.60 | 2.5 | 12.5 | 5.0 | 0.06 | 30.6 | +1.93 | +1.55 | +23.7% |
| 0.65 | 2.5 | 12.5 | 5.0 | 0.06 | 30.6 | +2.29 | +1.79 | +28.0% |
| 0.70 | 2.5 | 12.5 | 5.0 | 0.06 | 30.6 | +2.64 | +2.03 | +32.3% |
| 0.80 | 2.5 | 11.2 | 4.5 | 0.06 | 34.0 | +2.91 | +2.51 | +39.6% |

**观察**：
- $p = 0.55$（KF-19 EMA aligned 实测量级）相比 DirRandom，Sharpe 提升 19%，参数仍偏宽（$K_S=3.5$）；
- $p = 0.60$ 是**实盘参数拐点**：$K_S^\ast$ 从 3.5 降到 2.5（进入实盘常用区间），Sharpe 跃升至 1.55；
- $p \ge 0.60$ 后 $K_S^\ast$ 钉在 2.5，继续提升 $p$ 主要放大单笔 $E_{\text{net}}$ 和 Sharpe，参数不再大幅变化；
- $R^\ast$ 在 $p \le 0.70$ 顶到网格上界 5.0，说明方向 edge 不强时仍需极端非对称；$p = 0.80$ 时 $R^\ast$ 首次降到 4.5，符合命题 10.8 的定性预测。

**工程含义**：
1. **DirRandom 下 $K_S^\ast$ 顶到网格上界（3.5–4.0 ATR）**不是数学错误，是无方向 edge 的必然——紧止损会被噪声扫损；
2. **方向识别器的核心价值不仅是提高胜率，更是把参数从"理论最优（宽止损 + 大 RR）"拉回"实盘可用"**；
3. KF-19 的 EMA aligned 信号（$p \approx 0.55$）尚不足以单独让 $K_S^\ast$ 进入实盘区间，需要叠加更强信号或跨周期确认才能达到 $p \ge 0.60$；
4. **$p = 0.60$ 可作为方向识别器的工程 KPI**（类比通道 B 的 $\mathrm{se} \le 0.05$）——达到此阈值后 $K_S^\ast \le 2.5$ ATR，实盘可执行。

> **📚 文献批注（方向概率框架）**：命题 10.6 的 $p$-混合是对 Shepp (1969) "未知漂移 sign" 框架的有限 edge 推广——Shepp 设定先验 $p = 1/2$（对称），本推广允许 $p \in [1/2, 1]$ 任意非对称先验。Ekström & Lindberg (2013) 在 partial information 下也处理了非对称先验，但未与双 barrier 塑形参数优化结合；命题 10.8 的参数单调性结论为原创，基于玉米 1h 实测分布数值验证。
...
**【$p$-混合期望（通道 A/B 混合 · §10.5）】**

$$
\boxed{\;E_{\text{gross}}^{(p)}(x) = p \cdot g(+2x) + (1-p) \cdot g(-2x)\;}
$$
