...
| 诊断透传 | `diagnostics_json`（真实 alpha / risk / execution 三层，过滤占位） |
| 执行诊断 | `exit_reason`、`mae`、`mfe` |
...
本轮已优先落地持仓区间内的 MAE / MFE，而不是平仓后的 post-close 统计：

| 字段 | 状态 | 说明 |
|------|------|------|
| `holding_bars` | 已落地 | 按开仓到平仓区间内 K 线数量统计 |
| `mae` | 已落地 | long 取开仓后最大向下不利波动，short 取最大向上不利波动 |
| `mfe` | 已落地 | long 取最大向上有利波动，short 取最大向下有利波动 |

这些字段属于交易生命周期诊断，当前由 clearing 基于成交记录和回测 K 线派生，写入 `trade_clearings`，再由 analytics 聚合到 `clearing_diagnostics.json`。

post-close 一小时后验统计仍为后续扩展，不在本轮完成范围。
...
- report JSON schema：`clearing_diagnostics.schema.json`；
- 前端消费字段契约：`ClearingDiagnostics`；
- 数据导出格式：`clearing_diagnostics.json`；
- analytics artifact 入口：`build_clearing_diagnostics_for_run`；
- diff 工具基础形态：`build_exit_policy_diff`。

这些仍属于一期最小消费入口，不代表 analytics 二期已经完成。

## 19. 更新后的第一阶段实施顺序

| 顺序 | 工作 | 当前状态 |
|------|------|----------|
| 1 | 盘点 backtest 中现有配对、手续费、滑点和 PnL 计算位置 | 已完成实现迁移所需范围 |
| 2 | 新增 `workspace/clearing` 业务域 | 已完成 |
| 3 | 新增 `workspace/cli/workflows/clearing.py` | 已完成 |
| 4 | 在 `RunFinalizer` 中统一触发 clearing workflow | 已完成 |
| 5 | data 层增加 `trade_clearings`、`account_ledger_entries`、`position_ledger_entries` | 已完成 |
| 6 | 从 `backtest_trades` 读取成交与 reason 信息 | 已完成 |
| 7 | 实现 FIFO open lot 配对 | 已完成 |
| 8 | 支持部分平仓和一笔平仓拆多条 clearing record | 已完成 |
| 9 | 支持回测结束未平仓强制平仓 | 已完成 |
| 10 | 计算 trade-level gross / net PnL 和成本 | 已完成一期 |
| 11 | 生成 event-level account ledger / position ledger 骨架 | 已完成一期 |
| 12 | 回填或更新 backtests 统计字段 | 已完成一期 |
| 13 | 为 post-close 后验统计预留字段和线性处理边界 | 部分完成：已落地持仓区间 MAE / MFE；post-close 统计后置 |
| 14 | 补充 long / short / 部分平仓 / 强平 / 成本 / 汇总 / ledger 测试 | 基本完成，并补充了诊断透传、warning、report 导出和契约测试 |
...
## 23. 当前完成度评估（2026-06-30）

Clearing 一期本轮可以评为**基本完成**。

已完成：

- 独立 `workspace/clearing` 业务域；
- clearing workflow 与 `RunFinalizer` 触发链路；
- `trade_clearings`、`account_ledger_entries`、`position_ledger_entries` 三张表；
- FIFO open lot 配对；
- 部分平仓和一笔平仓拆多条 clearing record；
- 回测结束未平仓强制平仓；
- trade-level gross / net PnL、commission、slippage cost；
- account / position ledger event-level 骨架；
- backtests summary 回填；
- 真实 `decision_payload.diagnostics` 透传到 `trade_clearings.diagnostics_json`；
- 占位 diagnostics 过滤；
- 缺推荐字段 warning；
- holding bars、MAE / MFE 派生；
- 强平 `exit_reason`；
- report 查询、JSON 导出、schema 契约和前端消费入口。

仍未完成或后置：

- 完整 margin / daily settlement；
- 完整 production-grade position book；
- clearing 状态字段和可观测状态机；
- 独立 CLI 重跑命令和用户交互入口；
- post-close 1h 后验统计；
- 精确 slippage attribution（目标价 vs 实际成交价）；
- 多账户、实盘或 paper trading 对账；
- 旧 `backtest_trades` 派生字段的彻底语义迁出。

下一步不建议继续扩大 clearing 的账本复杂度；更高收益的方向是让 Alpha / Risk 层开始真实填字段，使已落地的 `diagnostics_json` 和 `clearing_diagnostics.json` 能产生解释价值。
...
除以上问题外，当前方案已经完成第一阶段主体实现；剩余事项主要是 post-close 后验统计、reconciliation warning、状态字段、生产级账本和更完整的 report contracts 后续增强。

## 25. Contracts 兼容与后续 Issue 备案

### 25.1 当前契约处理

本轮没有直接导出 `trade_clearings`、`account_ledger_entries` 或 `position_ledger_entries` 明细表，但已经新增一个 analytics 聚合产物：
...
- `trades.json` 继续输出现有字段，保持旧前端兼容；
- `backtests.json` / `summary.json` 字段名保持稳定；
- `trade_clearings`、`account_ledger_entries`、`position_ledger_entries` 暂不作为明细 report artifact 直接导出；
- `clearing_diagnostics.json` 作为可选 run artifact，旧 run 目录缺失时 contracts 校验跳过，存在时必须符合 schema；
- 后续如需导出明细，再新增 `clearings.schema.json` 或升级 `trades.schema.json`。

因此，本轮不破坏现有 contracts，同时为新增结构诊断产物补上了契约护栏。

### 25.2 `trades.json` 兼容口径
...
权威交易级清算结果只存在于 `trade_clearings`，不通过旧 `trades.json` 表达。`account_ledger_entries` 和 `position_ledger_entries` 是数据库事实层骨架，明细导出契约仍后移到后续 analytics-reporting 阶段。
